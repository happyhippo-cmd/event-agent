...
def build_restaurant_query(taste_context: dict[str, Any]) -> str:
...
def run_restaurant_agent(
    ...
    query = build_restaurant_query(taste_context)
...
def run_restaurant_agent_for_state(state: KDiveState, top_k: int = DEFAULT_TOP_K) -> KDiveState:
    """Run Restaurant when supervisor routed to restaurant or legacy foodie."""
...
def run_nearby_restaurant_agent(
    ...
    try:
        ...
        return {
            ...
            "query": query,
            "candidates": [],
            "message": f"Restaurant DB 조회 중 오류가 발생했어요: {exc}",
        }
...
def recommend_nearby_foods_for_places(
    ...
    try:
        ...
        return {"status": "error", "foods": [], "message": f"Restaurant DB 조회 중 오류가 발생했어요: {exc}"}
    ...
    foods = _apply_llm_food_curations(
        foods,
        {
            "current_keywords": [place.get("name") for place in anchors if place.get("name")],
            "place_type_keywords": [food.get("genre") for food in foods if food.get("genre")],
        },
        "선택한 관광지 근처 맛집 추천",
    )
    return {
...
def _apply_llm_curations(
    ...
    if not candidates:
        return candidates
    try:
        from .curate import curate_restaurant_items

        curations = curate_restaurant_items(candidates, taste_context, query, len(candidates))
    except Exception:
        return candidates
    if not curations:
        return candidates
    return [
        candidate.model_copy(update={"curation": curations.get(candidate.kakao_place_id, candidate.curation)})
        for candidate in candidates
    ]


def _apply_llm_food_curations(
    ...
    try:
        ...
        curations = curate_restaurant_items(foods, taste_context, query, len(foods))
    except Exception:
        return foods
    if not curations:
        return foods
    return [
        {**food, "curation": curations.get(str(food.get("id") or food.get("key") or food.get("name")), food.get("curation", ""))}
        for food in foods
    ]
...
def _food_payload(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "key": f"restaurant::{row.get('kakao_place_id') or name}",
